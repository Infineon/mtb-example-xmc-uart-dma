...
[Provide feedback on this code example.](https://cypress.co1.qualtrics.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[mtb-example-xmc-uart-dma](https://github.com/Infineon/mtb-example-xmc-uart-dma)" application with the desired name "Uart_Dma" configured for the *KIT_XMC48_RELAX_ECAT_V1* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_XMC48_RELAX_ECAT_V1 --app-id mtb-example-xmc-uart-dma --user-app-name Uart_Dma --target-dir "C:/mtb_projects"
   ```
...
The implementation is realized by using a UART resource. UART is configured by **uart 1.0** personality in **Device Configurator** tool where UART features, such as baudrate, data bits, and frame length are selected.
...
To transmit the data from the source to the user via UART, DMA and the UART interface are initialized. Next, the data is streamed from the first source. After the `XMC_DMA_CH_EVENT_BLOCK_TRANSFER_COMPLETE` event has occurred, data is streamed from the second source.
...
![](images/uart_personality.png)

<br>

**Figure 3. USIC (UART) settings**

![](images/uart_conf_1.png)
...
Document title: *CE231994* - *XMC&trade; MCU: UART DMA*
 ...
 1.0.0   | Updated to support ModusToolbox&trade; v2.3
